...
public class GameplaySettingsUITest : MonoBehaviour
{
    [Header("UI Elements")]
    ...
    private void GenerateUI()
    {
        ...
        foreach (var pair in metadata)
        {
            ...
            {
                case "slider":
                    float sliderValue = SettingsManagerTest.Instance.GetSetting<float>("gameplay", key);
                    CreateSlider(param, key, sliderValue);
                    break;

                case "toggle":
                    bool toggleValue = SettingsManagerTest.Instance.GetSetting<bool>("gameplay", key);
                    CreateToggle(param, key, toggleValue);
                    break;

                case "dropdown":
                    string dropdownValue = SettingsManagerTest.Instance.GetSetting<string>("gameplay", key);
    ...
    private void CreateSlider(JObject param, string key, float currentValue)
    {
        ...
        slider.Setup(
            ...
            value =>
            {
                SettingsManagerTest.Instance.SetOverride("gameplay", key, value);
                ApplyAndSave();
            },
    ...
    private void CreateDropdown(JObject param, string key, string currentValue)
    {
        ...
        dropdown.Setup(param["label"]?.ToString(), options, selectedIndex, index =>
        {
            ...
            SettingsManagerTest.Instance.SetOverride("gameplay", key, selected);
            ApplyAndSave();
        });
    }
